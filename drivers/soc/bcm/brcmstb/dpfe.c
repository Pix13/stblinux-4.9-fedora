...
/* Old API v2 firmware commands, as defined in the rev 0.61 specification, we use
 * a version set to 1 to denote that it is not compatible with the new API v2 and
 * onwards.
 */
...
/* API v2 firmware commands, as defined in the rev 0.8 specification, named
 * new v2 here
 */
...
static int brcmstb_dpfe_download_firmware(struct brcmstb_dpfe_priv *priv)
{
	...
	ret = request_firmware_direct(&fw, priv->dpfe_api->fw_name, dev);
	/*
